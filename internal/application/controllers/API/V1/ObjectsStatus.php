...
class ObjectsStatus extends ApiToken
{
    ...
    /**
     * GET /ObjectsStatus
     * Get current status of all user objects
     */
    public function getObjectsStatus()
    {
        $url = 'https://connect.traxroot.com/api/ObjectsStatus';
        $res = $this->requestCurl($url, 'GET');
        $this->_respond($res);
    }

    /**
     * GET /ObjectsStatus/{id}
     * @param int $id
     */
    public function getObjectsStatusById($id = null)
    {
        if (!$id) {
            return $this->_badRequest('Missing object ID');
        }

        $url = 'https://connect.traxroot.com/api/ObjectsStatus/' . intval($id);
        $res = $this->requestCurl($url, 'GET');
        $this->_respond($res);
    }
